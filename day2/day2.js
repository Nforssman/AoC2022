...
const DetermineScorePart1 = (round) => {
...
const DetermineScorePart2 = (round) => {
  const [elf, me] = round.map((x) => x.split(",")).map(String);
...
const part1 = () => {
  return parseInput
    .map((score) => DetermineScorePart1(score))
    .reduce((prev, curr) => prev + curr, 0);
};

const part2 = () => {
  return parseInput
    .map((score) => DetermineScorePart2(score))
    .reduce((prev, curr) => prev + curr, 0);
};
console.log(`Del 1: ${part1(parseInput)}`);
console.log(`Del 22: ${part2(parseInput)}`);
